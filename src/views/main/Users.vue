<template>
  <div class="p-5 xl:px-0">
    <transition name="fade" mode="out-in">
      ...
      <ul v-else class="mt-5">
        <li v-for="user in users" :key="user.id" class="flex items-center justify-between py-2 border-b">
          ...
          <a :href="`mailto:${user.email}`" :title="user.emailVerified ? 'Verified' : 'Not Verified'"
    ...
    <transition name="fade">
      <BasePagination path="users" :meta="meta" :links="links" action="paginateUsers" :store="store"
